...
int main() {
	...
	if(scelta == 1) {
		...
		std::cout << "Il testo criptato � stato salvato in: ./encripted.txt";
		keys.close();
		input.close();
		output.close();
		system("PAUSE");
		...
		std::cout << "Il testo decriptato � stato salvato in: ./decripted.txt";
		keys.close();
		input.close();
		output.close();
		system("PAUSE");
		...
		std::cout << "Una nuova Key � stata generata nel file: ./Key.txt";
		system("PAUSE");
	} else exit(0);
